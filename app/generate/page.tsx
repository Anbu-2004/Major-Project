...
export default function Generate() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(
        "An error occurred while generating Learning Cards. Please try again."
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const saveFlashcards = async () => {
    ...
    if (!trimmedSetName) {
      toast.error("Please enter a name for your Learning Cards set.");
      return;
    }
    ...
    try {
      ...
      toast.success("Learning Cards saved successfully!");
      onClose();
      setSetName("");
      ...
      toast.error(
        "An error occurred while saving Learning Cards. Please try again."
      );
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="py-8 px-4 mx-auto flex flex-col justify-start items-center w-full text-center lg:py-16 lg:px-12">
          <div className="max-w-screen-md mb-8 lg:mb-16">
            <h2 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">
              {flashcards.length < 1
                ? "Enter your subject or theme for Learning Cards"
                : `Generated Learning Cards for "${text}"`}
            </h2>
            ...
            {flashcards.length < 1 && (
              <>
                ...
                <Button
                  ...
                  disabled={!isLoaded}
                >
                  {isLoading ? "Generating Learning Cards" : "Generate Flashcards"}
                </Button>
              </>
            )}
        ...
        <ModalContent>
          {(onClose) => (
            <>
              ...
              <ModalBody>
                <p>
                  Are you sure you want to discard these Learning Cards? This action
                  cannot be undone.
                </p>
              </ModalBody>
              <ModalFooter>
